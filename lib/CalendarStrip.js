...
const month = ['Jan', 'Fev', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
...
class DateItem extends PureComponent {
  render() {
    ...
    const highlightBgColor = 'rgb(59, 140, 144)';
    const highlightLightBgColor = 'rgb(172, 213, 211)';
    const todayText = 'rgb(91, 162, 166)'
    const normalBgColor = 'transparent';
    const hightlightTextColor = '#fff';
    const normalTextColor = 'rgba(0,0,0,0.54)';
    const date = new Date()
    let today = false
    let before = false
    if (format(item, 'D')==format(date, 'D') && format(item, 'M')==format(date, 'M') && format(item, 'YYYY')==format(date, 'YYYY')) {
      today = true
    }
    if (item < date) {
      before = true
      ...
      <View style={styles.itemContainer}>
        ...
        >
          <View style={[
            styles.itemView,
            { paddingTop: 3 },
            {backgroundColor: highlight ? highlightBgColor : today ? highlightLightBgColor:normalBgColor},
            highlight && {shadowColor: 'rgba(0, 0, 0, 0.87)',
            shadowOffset: {
              width: 0,
              height: 1.5,
            },
            ...
            <Text style={[
              styles.itemDateText,
              {fontSize: 12,color: highlight ? hightlightTextColor : today ? todayText : before ? highlightBgColor:normalTextColor},
            
            ]}>{days[item.getDay()]}</Text>
            <Text style={[
              styles.itemDateText,
              {color: highlight ? hightlightTextColor : today ? todayText : before ? highlightBgColor:normalTextColor}
            ]}>{solar<10 ? '0'+solar:solar}</Text>
            {showLunar &&
              <Text style={[
  ...
  _renderHeader = () => {
    ...
    return (
      <View style={styles.header}>
        <View style={styles.wrapperMonthText}>
          <Text style={styles.monthText}>{month[selectedDate.getMonth()]}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerGoTodayButton: {
    borderRadius: 10,
    padding: 10,
    backgroundColor: 'rgb(59, 140, 144)',
